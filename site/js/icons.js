...
export function ring(frac, color, { track = '#253141', width = 5 } = {}) {
  ...
  return `<svg viewBox="0 0 100 100">
    ...
    <circle cx="50" cy="50" r="${RING_R}" fill="none" stroke="${color}" stroke-width="${width}"
      stroke-linecap="round" stroke-dasharray="${(f * CIRC).toFixed(1)} ${CIRC.toFixed(1)}"
      transform="rotate(-90 50 50)"/>
    <circle cx="50" cy="${50 - RING_R}" r="${width * 0.7}" fill="${color}"
      transform="rotate(${(f * 360).toFixed(1)} 50 50)"/>
  </svg>`;
}
...
export function compass(deg, frac, color = '#4fdc8b') {
  ...
  return `<svg viewBox="0 0 100 100">
    ...
    <g transform="rotate(${(deg || 0) + 180} 50 50)">
      <path d="M 50 ${50 - len} L 45 50 L 55 50 z" fill="${color}" opacity=".95"/>
      <path d="M 45 50 L 55 50 L 50 ${50 + len * 0.4} z" fill="${color}" opacity=".35"/>
    </g>
...
export function dial(frac, color = '#c9a6ff') {
  ...
  return `<svg viewBox="0 0 100 100">
    ...
    <circle cx="50" cy="50" r="${RING_R}" fill="none" stroke="${color}" stroke-width="5"
      stroke-linecap="round" stroke-dasharray="${(f * arc * CIRC).toFixed(1)} ${CIRC.toFixed(1)}"
      transform="rotate(120 50 50)"/>
    <line x1="50" y1="50" x2="${(50 + Math.sin(a + Math.PI) * -30).toFixed(1)}"
      y2="${(50 - Math.cos(a + Math.PI) * -30).toFixed(1)}" stroke="${color}" stroke-width="2.5"
...
export function droplet(frac, color = '#4f8bff') {
  ...
  return `<svg viewBox="0 0 100 100">
    ...
    <rect x="20" y="${top.toFixed(1)}" width="60" height="${(90 - top).toFixed(1)}"
      fill="${color}" opacity=".45" clip-path="url(#dropClip)"/>
    <path d="M 50 12 C 66 34 78 48 78 62 a 28 28 0 0 1 -56 0 C 22 48 34 34 50 12 z"
...
export function uvRing(uv) {
  ...
  return `<svg viewBox="0 0 100 100">${segs}
    <circle cx="50" cy="${50 - RING_R}" r="4.5" fill="#fff" transform="rotate(${(f * 360).toFixed(1)} 50 50)"/>
  </svg>`;
}
...
export function boltRing(frac, active) {
  ...
  return `<svg viewBox="0 0 100 100">
    ...
    ${active ? `<circle cx="50" cy="50" r="${RING_R}" fill="none" stroke="${col}" stroke-width="5"
      stroke-linecap="round" stroke-dasharray="${(f * CIRC).toFixed(1)} ${CIRC.toFixed(1)}"
      transform="rotate(-90 50 50)"/>` : ''}
    <g transform="translate(41 66) scale(1.05)" opacity="${active ? 1 : 0.35}">${BOLT(0, 0, 1, col)}</g>
  </svg>`;
}
